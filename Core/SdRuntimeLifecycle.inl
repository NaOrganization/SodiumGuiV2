...
namespace Sodium
{
	...
	inline void SdInstance::MarkSubmitted(SdWidgetRecord& record, SdWidgetId id, SdWidgetId parentId, SdResolvedKey resolvedKey, SdUtf8StringView debugKey)
	{
		...
		record.state.styleId = SdStyleTargetIds::Default;
	...
	{
		const float seconds = std::max(0.001f, context.styling.GetDesignTokenSet().GetMetric(SdDesignTokenIds::DurationFast));
		return {
