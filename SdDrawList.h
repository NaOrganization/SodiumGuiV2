...
namespace Sodium
{
	...
	private:
		...
		void PathFillConvex(const SdColor& color, const SdRect& clipRect);
		void PathStroke(const SdColor& color, const SdRect& clipRect, float thickness, bool closed);
		void PathFillConvexNoAA(const SdColor& color, const SdRect& clipRect, SdTextureHandle texture, const SdVec2& opaqueUv);
		void PathFillConvexAA(const SdColor& color, const SdRect& clipRect, SdTextureHandle texture, const SdVec2& opaqueUv);
		void PathStrokeNoAA(const SdColor& color, const SdRect& clipRect, SdTextureHandle texture, const SdVec2& opaqueUv, float thickness, bool closed);
		void PathStrokeAAThinTextured(const SdColor& color, const SdRect& clipRect, SdTextureHandle texture, float thickness, bool closed, SdUInt32 integerThickness);
		void PathStrokeAAThinGeometry(const SdColor& color, const SdRect& clipRect, SdTextureHandle texture, const SdVec2& opaqueUv, float thickness, bool closed);
		void PathStrokeAAThick(const SdColor& color, const SdRect& clipRect, SdTextureHandle texture, const SdVec2& opaqueUv, float thickness, bool closed);
		void BuildPolylineNormals(bool closed);
		void PrimRectWithUV(const SdRect& rect, const SdRect& uvRect, const SdColor& color, const SdRect& clipRect, SdTextureHandle texture);
	...
	public:
		...
		void AddLine(const SdVec2& a, const SdVec2& b, const SdColor& color, const SdRect& clipRect, float thickness = 1.0f);
		void AddTriangle(const SdVec2& a, const SdVec2& b, const SdVec2& c, const SdColor& color, const SdRect& clipRect, float thickness = 1.0f);
		void AddTriangleFilled(const SdVec2& a, const SdVec2& b, const SdVec2& c, const SdColor& color, const SdRect& clipRect);
		void AddQuad(const SdVec2& a, const SdVec2& b, const SdVec2& c, const SdVec2& d, const SdColor& color, const SdRect& clipRect, float thickness = 1.0f);
		void AddQuadFilled(const SdVec2& a, const SdVec2& b, const SdVec2& c, const SdVec2& d, const SdColor& color, const SdRect& clipRect);
		...
		void AddText(SdUtf8StringView text, const SdVec2& position, const SdColor& color, const SdRect& clipRect)
		{
			if (!sharedData)
				return;
			AddText(text, sharedData->defaultTextStyle, position, color, clipRect);
		}
		...
		void AddText(SdUtf8StringView text, const SdTextStyle& style, const SdVec2& position, const SdColor& color, const SdRect& clipRect)
		{
			...
			sharedData->fontBackend->DrainPendingUploads(drawData.uploads);
			...
			AddTextLayout(layout, color, clipRect);
		}

		void AddParagraph(SdUtf8StringView text, const SdTextStyle& style, const SdVec2& position, float maxWidth, const SdColor& color, const SdRect& clipRect)
		{
			if (!sharedData || !sharedData->fontBackend || text.empty())
				return;
		...
		void AddTextLayout(const SdParagraphLayout& layout, const SdColor& color, const SdRect& clipRect);

		SdDrawData& GetDrawData() noexcept { return drawData; }
		const SdDrawData& GetDrawData() const noexcept { return drawData; }
		SdDrawData& GetData() noexcept { return drawData; }
		const SdDrawData& GetData() const noexcept { return drawData; }
		SdDrawPacket BuildPacket(SdUInt32 packetVersion = 0) const noexcept { return drawData.BuildPacket(packetVersion); }
	};
