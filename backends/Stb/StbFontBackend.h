...
{
	struct SdStbFontBackendConfig final
	{
		SdUInt32 atlasWidth = 1024;
		SdUInt32 atlasHeight = 1024;
	};

	class SdStbFontBackend final : public ISdFontBackend
	{
	private:
	...
	public:
		using Config = SdStbFontBackendConfig;

		bool Initialize(const Config& backendConfig = {});
		void Shutdown();
		bool IsInitialized() const noexcept override { return fallbackFont.IsValid(); }
