...
namespace
{
	...
	struct ExampleApplication final
	{
		...
		{
		public:
			void OnUpdate(Sodium::SdUpdateContext& context, bool& windowOpen, bool& optionEnabled, int& clickCount, UINT64 frameCount, double liveFps, Sodium::SdTextureHandle fontAtlasTexture)
			{
				if (!windowOpen)
					return;
				...
				context.ui.DeclareKeyed<Sodium::SdWindow>("main_window", "SodiumGUI Demo", windowOpen, windowOptions, [&](Sodium::SdUi& ui)
				{
					...
					char buffer[64] = {};
					std::snprintf(buffer, sizeof(buffer), "Button clicks: %d", clickCount);
					ui.Declare<Sodium::SdText>(buffer);
					...
					ui.Declare<Sodium::SdImageViewer>(fontAtlasTexture, imageOptions);
				});
			}
		};
